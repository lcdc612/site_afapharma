...
<div id="box"><!-- INICIO DIV BOX -->
  ...
  <div id="content"><!-- INICIO DIV CONTENT -->
    ...
    <div id="conteudo"><!-- INICIO DIV CONTEUDO -->
       ...
       <div class="titulo">Empresa | Cadastrar Imagens<!--<p class="codigo"><strong>Giovani Distribuidora</strong></p>--></div>
        ...
        <?php if(isset($_POST['cadastrar_post']) && $_POST['cadastrar_post'] == 'cad'){
		 ...
		 if(!empty($nome) && in_array($type, $permitido)){
				...
				Redimensionar($tmp, $name, 1280, $pasta);
							 
			 	$cadastrar_noticias = mysql_query("INSERT INTO cad_midias_empresa (obraid, nomeimg) VALUES ('$PostIDInsert', '$name')") or die(mysql_error());
	...
	?>
             ...
             <fieldset>
                <label>
                    <input type="file" name="thumb" class="inputimg"/>					
					 <input type="submit" value="Clique Aqui para Enviar" name="Cadastrar" />
                </label> 
        ...
        <?php
			...
			if(!empty($_GET['delid'])){
				...
				$deletar_img = mysql_query("DELETE FROM cad_midias_empresa WHERE nomeimg = '$imgIm'") or die(mysql_error());	
			}
			
			$postid = $_GET['id'];
			
			$SelecionaImagem = mysql_query("SELECT * FROM cad_midias_empresa WHERE obraid = '$postid' ORDER BY fotoid ASC") or die(mysql_error());
        ...
        <ul class="gblist">
			...
			?>
            <li>
                <img src="tim.php?src=midias/<?php echo $NomeImagem; ?>&w=81&h=63&q=100&zc=1" width="81" height="63" />
                <div class="action">
                    <a href="midias/<?php echo $NomeImagem; ?>" rel="shadowbox" title=""><img src="imgs/view.png" title="Ver" alt="Ver" /></a>
                    <a href="configs_cad_imagens.php?&id=<?php echo $postid; ?>&delid=<?php echo $FotoIdPost; ?>&img=<?php echo $NomeImagem; ?>" title="Exluir"><img src="imgs/no.png" title="Exluir" alt="Exluir" /></a>
                </div><!-- /action -->
            </li>
            <?php
            	}
